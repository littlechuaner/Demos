...
datalist$'m1'
datalist$'m0'
#Visulization traning data
data_visulize <- function(datalist){
  require(ggplot2)
  df_x = cbind.data.frame(datalist$'traindata' , datalist$'Ytrain')
  names(df_x) <- c("X1","X2","Y")
  ggplot(df_x, aes(x = X1 , y = X2, colour = Y)) + geom_point() 
}
data_visulize(datalist)
###
